...
{   
    ...
    void Update(){
        ...
        else if(target.position.x > -18f && target.position.x < 19.25f) { 
        ...
        else if(target.position.x >= 19.25f && target.position.x < 58.5f) { 
            camPosition = new Vector3(38f,  -2f, -10f); 
            transform.position = Vector3.SmoothDamp(gameObject.transform.position, camPosition, ref velocity, dampTime);
        }
        else if(target.position.x >= 58.5f && target.position.x < 93.75f) { 
            camPosition = new Vector3(78f, -2f, -10f); 
            transform.position = Vector3.SmoothDamp(gameObject.transform.position, camPosition, ref velocity, dampTime);
        }
        else if(target.position.x >= 93.75f) { 
